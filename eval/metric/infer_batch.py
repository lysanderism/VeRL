...
def _get_message(obj_dict):
    formatted_text = f"<|TEXT_UNDERSTANDING_START|>{obj_dict['text']}<|TEXT_UNDERSTANDING_END|>"
    # Tokenize the text
    message = [
    ...
    for i in tqdm(range(0, len(datas), batch_size)):
        ...
        with accelerator.split_between_processes(batch_messages) as messages, accelerator.split_between_processes(batch_ids) as batch_id:
            ...
            outputs = model.generate(
                ...
                temperature=0.75,   #  Controls randomness in output
            )
...
# accelerate launch --multi_gpu ./LLaSA_training/infer_batch.py --model_path "HKUSTAudio_model/Llasa-1B" --data_file "LLaSA_training/test_en1.json" --out_audio LLaSA_training/eval/llasa_1b_base --batch_size 8
# accelerate launch --multi_gpu ./LLaSA_training/infer_batch.py --model_path "hf_model" --data_file "LLaSA_training/test_en1.json" --out_audio LLaSA_training/eval/llasa_3000 --batch_size 8